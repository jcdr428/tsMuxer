...
uint32_t TS_program_map_section::serialize(uint8_t* buffer, int max_buf_size, bool addLang)
{
    ...
    *LengthPos2 = my_htons(0xf000 + bitWriter.getBitsCount() / 8 - beforeCount2);
    ...
    for (PIDListMap::const_iterator itr = pidList.begin(); itr != pidList.end(); ++itr)
    {
        ...
        bool insertDoVi = (itr->second.m_pid >> 4 == 0x101) && !(V3_flags & 0x40) && (V3_flags & 0x04);
        ...
        if (insertDoVi)
        {
            setDoViDescriptor(bitWriter, itr->second.m_pid, DV_EL);
...
void TS_program_map_section::setDoViDescriptor(BitStreamWriter& bitWriter, int PID, bool DV_EL)
{
    ...
    bitWriter.putBits(
        7, (HDR ? (PID == 0x1011 ? 7 : (DV_EL ? 7 : 8)) : (PID == 0x1011 ? 4 : (DV_EL ? 4 : 5))));  // profile
